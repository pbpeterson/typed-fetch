...
const { response, error } = await typedFetch("https://api.example.com/users/123");
...
const { response, error } = await typedFetch<User>("https://api.example.com/users/123");
...
const { response, error } = await typedFetch("https://api.example.com/legacy");
...
const { response, error } = await typedFetch("https://api.example.com/users");
...
else if (!error) await response.body?.cancel();
```
...
const { response, error } = await typedFetch("https://example.test/users/1");
...
- **Under `require()`, a consumer subclass that puts an accessor on `name`, `status`, or `statusText` throws.** The same downlevelling turns the base class's field initializers into assignments, and an assignment to an inherited getter throws `TypeError`. Under `import` the identical subclass does not throw — but it does not work either: the base class defines the field over the accessor, so the getter never runs and `error.name` reads `"NotFoundError"` rather than the subclass's value. An accessor on those three members is unsupported in both formats, and only one of them says so. Declare them as fields, as the [subclass examples](#clone-a-consumer-subclass) do.
...
These guides state the breaking changes of each major release.
