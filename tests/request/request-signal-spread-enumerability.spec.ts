...
describe("the third condition on the entry a spread carries", () => {
...
describe("the caller aborts and the envelope reports a success", () => {
...
describe("the entry written on a branch with no signal to carry", () => {
...
describe("what the signal-spread pins can still fail for", () => {
  test("both pins that name the spread pass against the implementation round 19 refuted", () => {
    ...
    // failures. Only `tests/request/request-signal-spread-custody.spec.ts` fails, so
