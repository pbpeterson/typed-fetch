...
describe("test IDs", () => {
  test("no ID names two tests, in any spec file", () => {
    // Every suite, not this directory. The specs live in one group folder per
    // subject, so a walk that stopped at its own folder would ask the question
    // of a twelfth of the repository and still report perfect uniqueness.
    ...
    const walk = (dir: URL, prefix: string): void => {
      for (const entry of readdirSync(dir, { withFileTypes: true })) {
        if (entry.isDirectory()) {
          ...
          specFiles.push(`${prefix}${entry.name}`);
        }
      }
    };
    walk(testsDir, "");
    ...
    // nothing would report perfect uniqueness. The floor sits above the
    // largest single group, so a walk that stopped recursing fails here
