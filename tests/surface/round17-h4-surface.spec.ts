...
warnWhenDistMissing("round17-h4-surface", distExists);
...
describe.skipIf(!distExists)(
  ...
  () => {
    ...
    test("a transport reads no `fetch` extension off the init it is handed", async () => {
      ...
      // tests/request/round17-h1-request.spec.ts, which two lanes wrote
...
describe.skipIf(!distExists)("the internal script surface reaches no consumer", () => {
  ...
  test("`pnpm verify-pack` refuses a manifest that carries one of the new spec files", async () => {
    ...
    expect(() =>
      gate.verifyPackManifest([...shipped, "scripts/round16-docs-entry.spec.mjs"], 15),
    ).toThrow(/scripts\/round16-docs-entry\.spec\.mjs {2}\(test file\)/);
  });
...
// so tests/surface/round16-h4-surface.spec.ts pins this list to these two
...
describe("the coverage exclusion pin, driven against a grown exclusion list", () => {
  ...
  function pinExitCode(extra: string): number {
    ...
    try {
      ...
      copyFileSync(
        join(REPO_ROOT, "tests/surface/round16-h4-surface.spec.ts"),
        join(root, "tests/surface/round16-h4-surface.spec.ts"),
      );
      for (const entry of [
        "CHANGELOG.md",
        "README.md",
