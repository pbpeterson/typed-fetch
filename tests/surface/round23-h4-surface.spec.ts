...
describe("R23-H4-03 — the direction obligation at the release boundary", () => {
  ...
  test("the release gate reads that declaration out of the DATED section", () => {
    ...
    const reader = repoText("tests/surface/round19-h4-surface.spec.ts");
    ...
    expect(repoText("scripts/round21-h4-gate.spec.mjs")).toContain(
...
// sentence, and `scripts/round23-h4-gate.spec.mjs` carries its failing drive;
...
describe("the four sentences round 22 wrote", () => {
  ...
  test("CONTRIBUTING: a spec runs the Bun smoke wherever Bun and a built dist both exist", () => {
    ...
    const spec = repoText("tests/surface/round21-h4-surface.spec.ts");
    // The needle is BUILT, never written out. `round22-h4-surface.spec.ts`
  ...
  test("CONTRIBUTING: the roster sentence names two spellings and quantifies over all of them", () => {
    ...
    //   both.
    //
    // The drives are in `scripts/round23-h4-gate.spec.mjs`. These rows pin the
