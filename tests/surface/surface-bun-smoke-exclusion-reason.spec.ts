...
warnWhenDistMissing("surface-bun-smoke-exclusion-reason", distExists);
...
// `tests/surface/surface-runtime-smoke-commands.spec.ts` — written in the SAME round —
...
describe("CONTRIBUTING's reason for dropping the Bun smoke from the threshold", () => {
  ...
  test("EVIDENCE: a spec that spawns the smoke is guarded only by whether Bun exists", () => {
    const round21 = repoText("tests/surface/surface-runtime-smoke-commands.spec.ts");
  ...
  test("the reason CONTRIBUTING gives agrees with what the suite does", () => {
    ...
    const self = "tests/surface/surface-bun-smoke-exclusion-reason.spec.ts";
    ...
    expect(
      ...
      "CONTRIBUTING.md gives a REASON for dropping `scripts/smoke/bun.mjs` from the " +
        ...
        "whole point of it. `tests/surface/surface-runtime-smoke-commands.spec.ts` runs " +
