...
warnWhenDistMissing("surface-release-gate-roster", distExists);
...
describe.skipIf(!distExists)("the `[Unreleased]` block, against semver rule 8", () => {
  test("VERIFIED: the block claims only the direction that has a witness", () => {
    ...
    // `surface-changelog-direction-witnesses.spec.ts` the record is shorter than 2.0.1's on 2,264
    ...
    // `surface-changelog-direction-witnesses.spec.ts` > "every direction the block names is a
...
// of them has the same shape: `tests/surface/surface-gate-script-exports.spec.ts` carries
...
describe("the coverage threshold pin, driven against a lowered threshold", () => {
  ...
  function pinExitCode(anchor: string, mutated: string): number {
    ...
    try {
      ...
      copyFileSync(
        join(REPO_ROOT, "tests/surface/surface-gate-script-exports.spec.ts"),
        join(root, "tests/surface/surface-gate-script-exports.spec.ts"),
      );
      for (const entry of [
        ".github",
        "CHANGELOG.md",
  ...
  test("lowering a threshold from 100 to 0 leaves the pin green", () => {
    ...
    expect(
      ...
      "tests/surface/surface-gate-script-exports.spec.ts names its pin 'its thresholds are the four at " +
