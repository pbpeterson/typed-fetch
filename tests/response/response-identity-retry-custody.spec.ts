...
function foreignResponse(overrides: Record<PropertyKey, unknown> = {}): Response {
  const response: Record<PropertyKey, unknown> = {
    ...
    statusText: "OK",
    type: "basic",
    url: "https://response-identity-retry-custody.invalid/resource",
...
describe("identity caches and retries", () => {
  ...
  test("control: status, statusText, url, and headers stay cached through a proxy re-entry", async () => {
    ...
    const response = foreignResponse({
      ...
      get url() {
        reads.url += 1;
        return "https://response-identity-retry-custody.invalid/cached";
      },
      get headers() {
        reads.headers += 1;
    ...
    expect(error.url).toBe("https://response-identity-retry-custody.invalid/cached");
    await error.cancel();
  });
...
describe("success surface and active wrappers", () => {
