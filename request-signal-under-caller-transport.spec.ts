...
// round 9 left behind.
//
// Round 9 closed the plain `signal` read for the AMBIENT transport: the
// transport receives the `Request` whole and aborts on the signal in the
// platform's internal slot, so the read goes through `Request.prototype`'s
// accessor and an own data property can no longer supply a decoy. It kept the
// plain own read for CALLER CODE running as the transport, with this reason:
// caller code is admitted on the wider TAG check, so its input need not be a
// platform `Request` at all, and the accessor cannot be applied to a value that
// carries no slot.
//
...
// branch tested the TRANSPORT, not the input.

/**
 * A `Request` whose own `signal` property shadows the slot the platform aborts
 * on, with the slot's own signal reported beside it.
 *
 * `Request.prototype.signal` is a read-only accessor, so
 * `Object.defineProperty(request, "signal", { value })` shadows it for every
 * plain read. Middleware that decorates a `Request` writes this; nothing hostile
 * is required, and the transport never sees the decoy.
 */
function decoyedRequest(): { readonly request: Request; readonly governing: AbortSignal } {
  ...
  return { request, governing };
}

describe("the signal a platform Request contributes under a caller transport", () => {
  test("the plan reads the slot whichever transport runs", () => {
    // The read must not branch on the transport at all. The same decoyed
    // `Request`, planned twice, must answer with the slot's signal both times.
    const ambient = decoyedRequest();
    const caller = decoyedRequest();

    const ambientPlan = planRequest(ambient.request, {});
    const callerPlan = planRequest(caller.request, { fetch: recordingTransport().fetch });

    expect(ambientPlan.ambientTransport).toBe(true);
    expect(callerPlan.ambientTransport).toBe(false);
    expect(ambientPlan.signal).toBe(ambient.governing);
    expect(callerPlan.signal).toBe(caller.governing);
  });

  test("an own signal property cannot replace the signal a forwarding transport aborts on", async () => {
    // The consequence, end to end. The governing signal is the one in the
    // platform's internal slot, the same one round 9 established for the ambient
    // transport, because the transport below hands the `Request` straight back
    // to the platform. That is what a dependency-injection seam is for:
    // `{ fetch: (input, init) => ... }` is the shape the README's own `fetch`
    // option invites, and forwarding to the ambient implementation is its most
    // ordinary body — a wrapper that logs, or retries, or counts calls.
...
describe("what the setup phase takes off a Request it hands over", () => {
  test("exactly three reads: the tag, the url, and the signal", () => {
    ...
    const plan = planRequest(spy as unknown as Request, { fetch: recordingTransport().fetch });
    ...
    expect(plan.transportInput).toBe(spy);
