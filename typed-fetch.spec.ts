...
// ═══════════════════════════════════════════════════════════════════════════
// ROUND 6 — the test server's own contract.
//
// `?header=Key:Value` took the value from `split(":")[1]`, so every value
// carrying a colon was truncated: `Location:https://host/x` set `Location:
// https`. No spec passed such a value, so nothing was lying — it was a trap
// for the next test, which would have read `https` and blamed the library.
// ═══════════════════════════════════════════════════════════════════════════
...
describe("the test server sets the header value it was given", () => {
  test("a value that carries colons survives whole", async () => {
    ...
    const { response, error } = await typedFetch(url({ header: `Location:${target}` }));
  ...
  test("an entry with no colon sets nothing rather than an empty header", async () => {
    const { response, error } = await typedFetch(url({ header: "NoColonHere" }));
