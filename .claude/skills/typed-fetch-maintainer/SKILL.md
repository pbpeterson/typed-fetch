...
- `pnpm typecheck` uses `tsconfig.test.json` — it includes `tests/**/*.spec.ts` so `expectTypeOf` assertions are real. Plain `tsc --noEmit` skips them. The specs live in `tests/<subject>/`, one folder per subject: `redaction`, `request`, `response`, `errors`, `surface`, `envelope`. A spec added outside `tests/` still RUNS, because vitest globs recursively from the repository root, but it leaves the typecheck, which globs only `tests/`. That gate would keep passing while it stopped reading the file. Two tests read a source file through a CWD-relative path (`tests/errors/base-http-error.spec.ts` and `tests/errors/error-classes.spec.ts`, both reading `src/errors/base-http-error.ts`), so the suite must be run from the repository root.
- Tests hit a real local HTTP server (no mocks). Query params drive responses: `?status=`, `?body=`, `?header=Key:Value`.
- How far the library distrusts an injected `fetch` is settled by `docs/adr/0003-the-untrusted-fetch-conformance-boundary.md`: 28 in-scope rows, 8 permanently out of scope. `fixtures/hostile-fetch.ts` drives every row and `conformance.spec.ts` binds the two lists together, so a new guard without an ADR row fails the suite. Read it before adding a hostile-input defense.
