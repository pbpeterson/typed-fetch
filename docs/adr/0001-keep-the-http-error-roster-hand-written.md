...
like an obvious candidate for a code generator or a generic factory, and it has
been proposed more than once. This ADR records why it stays hand-written.

(Item 6 was removed after this decision: `statusCodeErrorMap` is now derived
from `httpErrors` rather than hand-maintained, because every class already
carries its own `static status`. That reduced six lists to five. It does not
change the argument below, which is about the remaining five — the ones that
carry information no other list holds.)
...
So the amortised cost of the hand-written roster is: six list edits, zero times
...
`tsc --declaration` for a hand-written class and for the most favourable
...
3. **`extends KnownHttpError` disappears.** The inheritance chain
   (`NotFoundError` → `KnownHttpError` → `BaseHttpError`) is simply absent
...
`statusCodeErrorMap` is now derived from `httpErrors` precisely because every
class already carries its own `static status`, so a second hand-maintained copy
could only ever _disagree_ with it. Derive projections; keep declarations.
...
1. **The roster starts moving.** Three or more status classes added or removed
   ...
   if it stops being closed, the amortised cost calculation flips.
...
4. **The per-class files stop being trivial.** Today each class is six lines of
   literals. If real per-class behaviour accumulates (custom messages,
