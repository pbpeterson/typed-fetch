...
describe("BaseHttpError.clone() — an owner this copy cannot see", () => {
  ...
  test("B18: release bypasses a hostile own body getter on a native branch", async () => {
    const response = new Response("payload", { status: 404 });
    const nativeClone = response.clone.bind(response);
    const nativeBodyGetter = Object.getOwnPropertyDescriptor(Response.prototype, "body")?.get;
    ...
    Object.defineProperty(response, "clone", {
      value() {
        branch = nativeClone();
        Object.defineProperty(branch, "body", {
          configurable: true,
          get() {
            throw cause;
          },
    ...
    let thrown: unknown;
    try {
      error.clone(() => null as unknown as NotFoundError);
    } catch (caught) {
      thrown = caught;
    }
    ...
    try {
      expect(thrown).toBeInstanceOf(TypeError);
      expect((thrown as Error).message).toMatch(/returned null instead of an error/);
      expect((thrown as Error).cause).not.toBe(cause);
      expect(branch?.bodyUsed).toBe(true);
      expect(await settlesWithin(error.cancel())).toBe("settled");
    } finally {
      if (branch && nativeBodyGetter) {
        const body = Reflect.apply(nativeBodyGetter, branch, []) as ReadableStream | null;
  ...
  test("B18b: release bypasses an own body value that hides the native branch", async () => {
    const response = new Response("payload", { status: 404 });
    const nativeClone = response.clone.bind(response);
    const nativeBodyGetter = Object.getOwnPropertyDescriptor(Response.prototype, "body")?.get;
    let branch: Response | undefined;
